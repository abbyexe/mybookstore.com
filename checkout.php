...
if(isset($_SESSION['id'])){
	...
	$sql = "SELECT customer.CustomerName, customer.CustomerUPI, customer.CustomerGender, customer.CustomerAddress, customer.CustomerEmail, customer.CustomerPhone, book.BookTitle, book.Price, book.Image, `order`.`DatePurchase`, `order`.`Quantity`, `order`.`TotalPrice`
...
<?php
	...
	echo "<tr><td>No.Number: </td><td>".$row['CustomerUPI']."</td></tr>";
...
$nameErr = $emailErr = $genderErr = $addressErr = $UPIErr = $contactErr = "";
$name = $email = $gender = $address = $UPI = $contact = "";
$cID;
								...
								if(!preg_match("/^[0-9 -]*$/", $contact)){
									...
									if (empty($_POST["gender"])){
										...
										if (empty($_POST["address"])){
											...
											$sql = "INSERT INTO customer(CustomerName, CustomerPhone, CustomerUPI, CustomerEmail, CustomerAddress, CustomerGender) 
											VALUES('".$name."', '".$contact."', '".$UPI."', '".$email."', '".$address."', '".$gender."')";
											$conn->query($sql);
 
											$sql = "SELECT CustomerID from customer WHERE CustomerName = '".$name."' AND CustomerUPI = '".$UPI."'";
											$result = $conn->query($sql);
											while($row = $result->fetch_assoc()){
												$cID = $row['CustomerID'];
											}
...
if(!isset($_SESSION['id'])){
	...
	echo 'UPI Number:<br><input type="text" name="UPI" placeholder="name@upi">';
	echo '<span class="error" style="color: red; font-size: 0.8em;"><?php echo $UPIErr;?></span><br><br>';
	...
	echo 'Mobile Number:<br><input type="text" name="contact" placeholder="9876543210">';
